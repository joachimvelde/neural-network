...
class Network:
    ...
    def train(self, count, load=False):
        ...
        for x in range(count):
            ...
            if (x / count * 100 + 1) % 1 == 0 and load:
                progress = math.ceil(x / count * 100 + 1)
                sys.stdout.write('\r')
                sys.stdout.write("[%-10s] %d%%" % ('=' * int(progress/10), progress))
                sys.stdout.flush()

        print('\n') # For scuffed printing^
    ...
    def final_out(self):
        final = copy.deepcopy(self.out())
        for row in range(len(final)):
            for col in range(len(final[row])):
                final[row][col] = round(final[row][col])
        return final
...
def main():
    # Let the output layer automatically match the training data?
    nn = Network([2, 3])
    mat_print(train_out)
    nn.train(1000, load=True)
    mat_print(nn.final_out())
